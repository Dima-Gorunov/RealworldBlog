...
const Likes = ({ Count = 3, Favorited = false, Slug = null, ...props }) => {
  const { slug } = useParams();
  const setFavorite = (value) => {
    if (!slug) {
      props.setFavoriteThunk(Slug, value);
    } else {
      props.setCurrentFavoriteThunk(slug, value);
    }
  };
  return (
    <div className={`${styles.likesContainer} ${!props.Auth && styles.disabled}`}>
      {Favorited ? (
        <svg
      ...
      <span className={styles.likesCount}>{Count || 0}</span>
    </div>
  );
};
